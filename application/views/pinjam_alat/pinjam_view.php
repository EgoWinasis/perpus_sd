	    ...
	    <div class="col-md-12">
	        <div class="box box-primary">
                <div class="box-header with-border"><?php if($this->session->userdata('level') == 'Petugas'){ ?>
                    <a href="transaksi_alat/pinjam"><button class="btn btn-primary">
                    ...
                    <table id="example1" class="table table-bordered table-striped table" width="100%">
						...
						<?php 
							...
							foreach($pinjam->result_array() as $isi){
									$kode_anggota = $isi['anggota_id'];
									$ang = $this->db->query("SELECT * FROM tbl_siswa WHERE kode_anggota = '$kode_anggota'")->row();
								...
								<td style="text-align:center;">
									<?php if($this->session->userdata('level') == 'Petugas'){ ?>
										<?php if($isi['tgl_kembali'] == '0') {?>
											<a href="<?= base_url('transaksi_alat/kembalipinjam/'.$isi['pinjam_id']);?>" class="btn btn-warning btn-sm" title="pengembalian buku">
										...
										<?php }?>
										<a href="<?= base_url('transaksi_alat/detailpinjam/'.$isi['pinjam_id'].'?pinjam=yes');?>" class="btn btn-primary btn-sm" title="detail pinjam"><i class="fa fa-eye"></i></button></a>
										<a href="<?= base_url('transaksi_alat/prosespinjam?pinjam_id='.$isi['pinjam_id']);?>" 
											onclick="return confirm('Anda yakin Peminjaman Ini akan dihapus ?');" 
											class="btn btn-danger btn-sm" title="hapus pinjam">
											<i class="fa fa-trash"></i></a>
									<?php }else{?>
										<a href="<?= base_url('transaksi_alat/detailpinjam/'.$isi['pinjam_id']);?>" 
