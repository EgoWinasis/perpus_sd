...
class Transaksi extends CI_Controller
{
	...
	public function pinjam()
	{
		...
		$this->data['nop'] = $this->M_Admin->buat_kode_pinjam('tbl_pinjam', 'PJ', 'id_pinjam', 'ORDER BY id_pinjam DESC LIMIT 1');
		$this->data['idbo'] = $this->session->userdata('ses_id');
	...
	public function prosespinjam()
	{
		...
		if (!empty($post['tambah'])) {
			...
			$cekBuku = $this->db->query("SELECT * FROM tbl_pinjam WHERE kode = '" . $kode_buku . "' AND status = 'Dipinjam'")->result_array();

			if (count($cekBuku) > 0) {
				if ($this->$_SESSION['pesan'] == null) {
					$this->session->set_flashdata('pesan', '<div id="notifikasi"><div class="alert alert-danger">
				<p> Gagal Pinjam Buku ! Buku Sedang dipinjam siswa lain</p>
				</div></div>');
				}
			} else {
	...
	{
		$buku = $this->M_Admin->get_tableid_edit('tbl_buku', 'kode_buku', $this->input->post('kode_buku'));
		$id = $buku->id_kategori;
		$row = $this->db->query("SELECT * FROM tbl_kategori WHERE id_kategori ='$id'");
		$kategori = $row->row();
		error_reporting(0);
		if ($buku->title != null) {
			echo '<table class="table table-striped">
